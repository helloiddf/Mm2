...
-- 🔥 추가된 에임봇 설정 변수
local Config = {
    Aimbot = false, Aimbot_UseFOV = true, Aimbot_WallCheck = true, Aimbot_Smoothness = 0.5,
    HitboxExpander = false, TriggerBot = false, TriggerBotFOV = 50, Wallbang = false, 
...
RunService.RenderStepped:Connect(function()
    if Config.DrawFOV or (Config.Aimbot and Config.Aimbot_UseFOV) then
...
local function GetAimbotTarget()
    ...
    local shortestDist = Config.Aimbot_UseFOV and Config.FOVSize or math.huge
    ...
    for _, p in ipairs(Players:GetPlayers()) do
        if p ~= LocalPlayer and p.Character and p.Character:FindFirstChild("Head") and p.Character:FindFirstChild("Humanoid") and p.Character.Humanoid.Health > 0 then
            ...
            -- 내가 살인마면 다른 모든 사람 타겟, 내가 무고한/보안관이면 살인마만 타겟
            if myRole == "Murderer" then
                if pRole ~= "Murderer" then targetValid = true end
            else
                if pRole == "Murderer" then targetValid = true end
            end

            if targetValid and IsVisible(p.Character.Head) then
...
AddHeader(left1, "🔥 God-Tier Aimbot")
AddToggle(left1, "🔴 Aimbot Enable (에임봇 켜기)", function(v) Config.Aimbot = v end)
AddToggle(left1, "👀 Use FOV (원 밖 무시)", function(v) Config.Aimbot_UseFOV = v end)
AddToggle(left1, "🧱 Wall Check (벽 뒤 무시)", function(v) Config.Aimbot_WallCheck = v end)
AddSlider(left1, "🎯 에임 부드러움 조절", 1, 10, 5, function(val) Config.Aimbot_Smoothness = val / 10 end)

AddHeader(left1, "Hitbox & Wallbang")
AddToggle(left1, "🧲 Hitbox Expander (대충 쏴도 맞음)", function(v) Config.HitboxExpander = v end)
AddToggle(left1, "🧱 Perfect Wallbang (벽뚫샷 모드)", function(v) Config.Wallbang = v end)

AddHeader(right1, "TriggerBot")
AddToggle(right1, "🔫 TriggerBot (자동 발사)", function(v) Config.TriggerBot = v end)
AddSlider(right1, "   ↳ Trigger 범위 조절", 5, 100, 50, function(val) Config.TriggerBotFOV = val end)
